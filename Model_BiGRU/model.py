...
class BiGRU(nn.Module):
    def __init__(self, config, vocab_size, word_embeddings, pos_vocab_size):#, pos_embeddings
        ...
        self.gru=nn.GRU(input_size = self.config.embed_size+self.config.pos_embed_size,
                            hidden_size = self.config.hidden_size,
                            num_layers = self.config.hidden_layers,
                            dropout = self.config.dropout_keep,
                            bidirectional = self.config.bidirectional)
    ...
    def forward(self, x, pos):#新增传参pos
        ...
        #lstm_out, (h_n, c_n) = self.lstm(embedded_all)#维度有变output, hn = rnn(input, h0)
        gru_out,h_n=self.gru(embedded_all)
